...
export default async function InventoryItemEditPage({ params }: PageProps) {
  ...
  if (!session.allowed) {
    redirect(`/login?next=/admin/inventario/items/${itemId || ""}`);
  }

  if (!itemId) {
    redirect("/admin/inventario?error=Ítem+inválido");
  }
  ...
  const categories: Array<Pick<InventarioCategoria, "id" | "nombre">> = categoriesData ?? [];
