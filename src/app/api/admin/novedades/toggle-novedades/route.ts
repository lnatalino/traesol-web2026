...
export async function POST(req: Request) {
  ...
  if (!id || !nextValue) {
    ...
    url.searchParams.set("error", "No se pudo actualizar el estado de novedades.");
  ...
  try {
    ...
    const { error } = await (supabaseService as any)
      .from("novedades")
      .update({ en_novedades: value, updated_at: new Date().toISOString() })
      .eq("id", id);
    ...
    const message = err?.message ? String(err.message) : "No se pudo actualizar el estado de novedades.";
    if (payload.redirectTo) {
