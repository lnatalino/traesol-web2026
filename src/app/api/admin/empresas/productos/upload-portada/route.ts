...
const PORTADAS_FOLDER = "empresas/productos/portadas";
...
async function uploadPortada(file: File) {
  ...
  const publicUrl = supabaseService.storage.from(PUBLIC_STORAGE_BUCKET).getPublicUrl(path);
  return { path, url: publicUrl.data.publicUrl };
}
